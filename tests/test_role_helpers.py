...
def test_validate_inputs_returns_manifest_with_schema_and_identity() -> None:
    manifest = validate_inputs(
        np.ones((2, 64)), horizon=8, past_covariates=np.ones((3, 64)), names=["a", "b"]
    )
    ...
    assert (manifest["horizon"], manifest["past_covariates"], manifest["future_covariates"]) == (8, 3, 0)
...
def test_validate_inputs_rejects_like_forecast() -> None:
    ...
    with pytest.raises(ValueError, match="future_covariates must contain exactly 72"):
        validate_inputs(np.ones(64), horizon=8, future_covariates=np.ones(71))
